...
| 名稱 | 用途 | 存放位置 |
|------|------|----------|
| `SYNC_SECRET` | Worker `/sync` 端點授權 | Cloudflare Worker secret、4 個 category repo 的 `AGENTKIT_SYNC_SECRET` |
...
SYNC_SECRET=<YOUR_SYNC_SECRET> \
  python3 scripts/backfill_index.py
```
...
```bash
cd agentkit-api
source ~/.nvm/nvm.sh && node node_modules/wrangler/bin/wrangler.js deploy
```
...
# 2. 更新 Worker
cd agentkit-api
source ~/.nvm/nvm.sh && echo "<NEW_SECRET>" | node node_modules/wrangler/bin/wrangler.js secret put SYNC_SECRET

# 3. 更新 4 個 category repo 的 GitHub Actions secret
for repo in agentkit-skills agentkit-prompts agentkit-mcp agentkit-plugins; do
  gh secret set AGENTKIT_SYNC_SECRET --repo "knew-inventai/$repo" --body "NEW_SECRET"
done
```
